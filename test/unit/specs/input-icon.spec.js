...
describe('InputIcon', () => {
  ...
  it('suffixIcon', () => {
    vm = createVue({
      template: `
        <el-input-icon suffix-icon="time"></el-input-icon>
      `
    }, true);
  ...
  it('prefixIcon', () => {
    vm = createVue({
      template: `
        <el-input-icon prefix-icon="time"></el-input-icon>
      `
    }, true);
    var icon = vm.$el.querySelector('.el-input__icon');
    expect(icon).to.be.exist;
  ...
  it('rows', () => {
    vm = createVue({
      template: `
        <el-input-icon type="textarea" :rows="3">
        </el-input-icon>
      `
    }, true);
  ...
  it('resize', async() => {
    vm = createVue({
      template: `
        <div>
          <el-input-icon type="textarea" :resize="resize"></el-input-icon>
        </div>
      `,
      data: {
        resize: 'none'
  ...
  it('autosize', async() => {
    ...
    var limitSizeInput = vm.$refs.limitSize.$refs.ElInput;
    var limitlessSizeInput = vm.$refs.limitlessSize.$refs.ElInput;
  ...
  it('focus', async() => {
    ...
    vm.$refs.input.$refs.ElInput.$on('focus', spy);
    vm.$refs.input.$refs.ElInput.focus();
  ...
  it('Input contains Select and append slot', async() => {
    vm = createVue({
      template: `
      <el-input-icon v-model="value" clearable class="input-with-select" ref="input">
    ...
    vm.$refs.input.$refs.ElInput.hovering = true;
      ...
      template: `
        <el-form :model="model" :rules="rules">
          <el-form-item prop="input">
            <el-input-icon v-model="model.input" :validate-event="false">
            </el-input-icon>
          </el-form-item>
        </el-form>
      `,
  ...
  describe('Input Events', () => {
    it('event:focus & blur', async() => {
      ...
      vm.$refs.input.$refs.ElInput.$on('focus', spyFocus);
      vm.$refs.input.$refs.ElInput.$on('blur', spyBlur);
    ...
    it('event:change', async() => {
      ...
      const spy = sinon.spy();
      vm.$refs.input.$refs.ElInput.$on('change', spy);
    ...
    it('event:clear', async() => {
      ...
      inputElm.focus();
      vm.$refs.input.$refs.ElInput.$on('clear', spyClear);
      await waitImmediate();
    ...
    it('event:input', async() => {
      ...
      const spy = sinon.spy();
      vm.$refs.input.$refs.ElInput.$on('input', spy);
      const nativeInput = vm.$refs.input.$refs.ElInput.$el.querySelector('input');
      nativeInput.value = '1';
  ...
  describe('Input Methods', () => {
    it('method:select', async() => {
      ...
      expect(vm.$refs.inputComp.$refs.ElInput.$refs.input.selectionStart).to.equal(testContent.length);
      expect(vm.$refs.inputComp.$refs.ElInput.$refs.input.selectionEnd).to.equal(testContent.length);

      vm.$refs.inputComp.$refs.ElInput.select();

      await waitImmediate();
      expect(vm.$refs.inputComp.$refs.ElInput.$refs.input.selectionStart).to.equal(0);
      expect(vm.$refs.inputComp.$refs.ElInput.$refs.input.selectionEnd).to.equal(testContent.length);
    });
  });
  ...
  it('sets value on textarea / input type change', async() => {
    vm = createVue({
      template: `
        <el-input-icon :type="type" v-model="val" />
      `,
      data() {
        return {
          type: 'text',
